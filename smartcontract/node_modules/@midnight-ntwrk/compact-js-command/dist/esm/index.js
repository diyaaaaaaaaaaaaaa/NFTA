...
if (isProcessRootModule()) {
    const cli = Command.run(Command.make('cptexec').pipe(Command.withDescription('Executes Compact compiled contracts from the command line.'), Command.withSubcommands([deployCommand, circuitCommand, maintainCommand])), {
        name: 'Compact Contract Execute',
        version: '0.0.0'
    });
    cli(process.argv).pipe(Logger.withMinimumLogLevel(LogLevel.None), Effect.provide(Layer.mergeAll(ConfigCompiler.layer.pipe(Layer.provideMerge(NodeContext.layer)), CliConfig.layer({ showBuiltIns: false }))), NodeRuntime.runMain({ disableErrorReporting: true }));
}
// #endregion
