...
import { ConfigError as EffectConfigError, Console, DateTime, Effect, Layer } from 'effect';
 ...
 */
export const ttl = (duration) => DateTime.now.pipe(Effect.map((utcNow) => DateTime.toDate(DateTime.addDuration(utcNow, duration))));
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const reportCausableError = (err) => Effect.gen(function* () {
    const buildCauseDocs = () => {
        const docs = [];
        const buildCauseDoc = (errOrDoc) => {
    ...
    let errorDoc = Doc.text(err.message);
    if (err.cause) {
        errorDoc = errorDoc.pipe(Doc.catWithLineBreak(Doc.annotate(Doc.text('(cause)'), Ansi.italicized)), Doc.catWithLineBreak(Doc.hsep([
            Doc.text('..'),
 ...
 */
export const reportContractConfigError = (err) => Effect.gen(function* () {
    if (err.cause && err.cause instanceof ConfigCompilationError.ConfigCompilationError) {
        return yield* reportCausableError({
            message: err.message,
            cause: Doc.annotate(Doc.text(err.cause.message), Ansi.italicized).pipe(Doc.catWithLineBreak(Doc.hsep([
                Doc.text('..'),
...
const reduceConfigError = (err) => EffectConfigError.reduceWithContext(err, undefined, {
 ...
 * @returns An `Effect` that writes `err` to the console.
 */
export const reportContractExecutionError = (err) => Effect.gen(function* () {
    if (EffectConfigError.isConfigError(err)) {
        ...
        if (errorType & ConfigErrorType.InvalidData || errorType & ConfigErrorType.MissingData) {
            yield* Console.log();
            yield* Console.log(Doc.render(Doc.vsep([
 ...
 * @category layers
 */
export const layer = (configProvider, zkBaseFolderPath) => Layer.mergeAll(ZKFileConfiguration.layer(zkBaseFolderPath), Configuration.layer).pipe(Layer.provideMerge(NodeContext.layer), Layer.provide(Layer.setConfigProvider(configProvider)));
/**
 ...
 */
export const invocationHandler = (handler) => (inputs) => Effect.gen(function* () {
    ...
    const contractRuntime = ContractExecutableRuntime.make(layer(CommandConfigProvider.make(contractModule.config ?? {}, InternalOptions.asConfigProvider(inputs)), moduleImportDirectoryPath));
    yield* handler(inputs, moduleSpec).pipe(Effect.provide(CompiledContractReflection.layer(moduleImportDirectoryPath).pipe(Layer.provideMerge(NodeContext.layer))), contractRuntime.runFork, Effect.catchAll(reportContractExecutionError));
}).pipe(Effect.catchAll(reportContractConfigError));
